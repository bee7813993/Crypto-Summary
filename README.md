...
##### 過年度分（クローラー画面へ手動インポートしたもの）

クロール結果は当年分しか持たない。過年度の公式 CSV をクローラー画面に手動インポート
していれば、その内容（`viewer_ledger.json` / `viewer_transfers.json`）も同期で取り込む。
PBR のデータをクローラー側で一元管理できる。

二重計上を防ぐため、取り込む範囲を 2 つの条件で絞っている:

- **クロールがカバーする期間より前だけ**取り込む。同じ期間はクロール結果を正とする
