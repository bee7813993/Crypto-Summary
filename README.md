...
再クロールでの訂正もそのまま反映される。旧システム期間（〜2026-03-02）や他ソースの
データには触れない。
...
2. 公式 CSV を通常どおり `pbr` で取り込む
3. 翌年の同期は前年に触れない（クロールの JSON は当年分のみを持つため）

クロール由来データはソース `pbr_crawl`、公式 CSV 由来は `pbr` と分かれているので、
パージは公式データに影響しない。

> 当年の同期では、手作りの CSV で取り込んでいたクロール期間の `pbr` 行も
> 一緒に洗い替えられる（同じデータの重複を避けるため）。その結果、当時の
> インポート履歴バッチは残るが取引数が減る。入出金履歴 CSV のバッチには
> 旧システム期間の入庫も含まれるので、UI からバッチごと削除しないこと。
> 過去年の同期では公式 CSV 由来の `pbr` には触れないので、パージ後に
> 古いクロール結果を同期しても公式データを失わない。
