...
| `nexo_spot` / `nexo_dnw` | Nexo Pro スポット取引 / 入出金（貯蓄・先物とは別サービス）|
...
```bash
PBR_CRAWL_DIR=C:/path/to/PBRLending-History-Check/outputs
PBR_VIEWER_URL=http://127.0.0.1:4173   # 省略可。書くとクローラー画面のタブが出る
```

**2. 利用者側**: インポート画面の「集計の設定」で
「PBR Lending クローラー連携を使う」をオンにする。

**既定では連携の UI を出さない。** 全員が PBR Lending の口座を持つわけではなく、
口座があっても年次の公式 CSV を取り込むだけならクローラーは要らないため。

| 利用者の状況 | 連携の UI |
|---|---|
| PBR を使っていない | 出ない |
| 年次の公式 CSV を取り込むだけ | 出ない |
| クローラー連携を使ったことがある | 出る（設定不要） |
| 明示的にオンにした | 出る |

オフにしても取り込み済みのデータは残る（表示の切り替えだけ）。年次パージで
`pbr_crawl` が空になっても、同期の記録があるので出続ける。

- 画面を開いたときに**新しいクロール結果があれば自動で取り込む**
- インポート画面の「PBR Lending クローラー同期」カードから手動実行もできる
- `PBR_VIEWER_URL` を書くと、サイドバーの「PBR クローラー」タブにクローラーの
  ビューア画面が開く（表示にはクローラー側のビューア起動が必要: `npm run viewer`）。
  書かなければタブは出ない — クラウド運用ではクロールは手元の役目なので不要
