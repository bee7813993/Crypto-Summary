...
> **この文書の範囲**
> ここに載っているのは、サービストークンで読める 6 本 + ヘルスチェックだけです。
  ...
  "assets": [
    {
      ...
      "prev_balance": "0.3",       // prev_date 時点の残高。取れなければ null
      "prev_value": "4200000",     // prev_balance × prev_price。取れなければ null
...
**`balance` が `"0"` の行が混ざることがあります。** その日に全部売った・送り出した
資産で、前日値を持たせるためだけに載っています（落とすと、受け取った側だけが増えて
見かけの利益が出ます）。`asset_count` / `priced_count` / `unpriced` はこの行を数えません
— いま持っている資産だけを数えます。翌日には消えます。
...
```
prev_value = 前日（prev_date 時点）の残高 × 前営業日の終値
day_change = value − prev_value
```

**残高も前日のもの**を使います（2026-08-28 に「いまの残高 × 前日終値」から変更）。
残高は数量そのものなので、いまの残高で評価すると入出庫が打ち消し合って消えてしまい、
`day_change` が価格変動しか表せなくなるためです。この定義なら `day_change` は
**その日に資産が実際にどれだけ増減したか**（入出金・売買を含む）を表し、
`/api/portfolio-history` の前日の点との差と一致します。AS 側の銘柄も同じ定義です。

今日 1 BTC 入金していれば、その入金額は前日比に現れます。
価格変動だけを見たいときは `prev_balance × (price − prev_price)` を使ってください。
...
`prev_price` / `prev_balance` / `prev_value` / `prev_date` は**セット**で `null` になります
（前日の残高は台帳から必ず出せるので、欠けるのは常に価格側の理由です）。
...
**指定日時点の残高（数量だけ）**。価格を引かないので CoinGecko を叩かず、SQL 1 本で返ります。
「1 週間前・30 日前と比べて数量がどれだけ動いたか」を利用側で組み立てるための素材です。

**クエリ**: `as_of`（`YYYY-MM-DD`、省略時は今日）/ `scope`（既定 `total`。
`account:<表示名>` / `asset:<シンボル>` も可。書式は `/api/portfolio-history` と同じ）
...
- 値は**その日の終わり（UTC）時点の累積残高**で、`/api/portfolio-history` の同じ日の
  点と同じ残高です（あちらは価格を掛けた評価額を返します）。
