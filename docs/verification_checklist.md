...
- [ ] ダッシュボードの「主な口座」は評価額上位 5 件のみ表示される
- [ ] ダッシュボードの「主な資産」は評価額上位 5 件のみ表示される
- [ ] 各カード下部の「すべての口座を表示（N）→」「すべての資産を表示（N）→」リンクで
      それぞれ口座別 / 資産別ページに遷移する（URL も `#accounts` / `#assets` になる）
- [ ] 件数が 5 件以下のときは「すべて表示」リンクが出ない
- [ ] 少額残高トグル・「価格未対応」注記は**資産別ページ**に移設されている
      （ダッシュボードには出ない）
- [ ] 資産別ページで「少額残高のトークンを表示/隠す」が機能し、再取得なしで切り替わる
- [ ] 口座別・資産別の一覧で行をクリックすると詳細に遷移し、**URL が更新される**
      （リロードで詳細が復元できる ＝ セクション10と整合）
- [ ] ダッシュボードの円グラフ（資産構成）は従来どおり表示される
...
- [ ] ダッシュボードに「資産推移」カードが表示される
- [ ] カード右上にレンジトグル **7D / 30D / 90D / 1Y / ALL** が並んでいる
- [ ] 初期表示で **90D** が選択されている（強調表示）
- [ ] 折れ線グラフが描画される（Y軸: 評価額、X軸: 日付）
- [ ] レンジトグルをクリックすると対応する期間に切り替わる
- [ ] 期間を変えても選択レンジが保持される（ページ遷移で90Dにリセット **されない**）
- [ ] データが存在しない場合は「取引データがありません」と表示される

### 14-2. 口座詳細ページ（口座別推移）

- [ ] 口座詳細ページに「評価額推移」グラフカードが表示される
- [ ] その口座の保有資産の評価額推移が折れ線で表示される
...
- [ ] 価格未対応の資産がある場合、グラフ下部に「価格未対応（評価額に未算入）: XX」と表示される
