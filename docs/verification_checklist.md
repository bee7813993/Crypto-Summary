...
> **カテゴリ別の取得範囲**: `spot` は現物約定＋入出金を取得。`linear`（USDT建て先物）は先物約定のみを取得。
> 入出金はウォレット全体の操作のため category に依存せず、spot 登録からのみ取得することで二重登録を防ぐ。
> 同一アカウントで spot と linear を両方登録する場合、入出金は spot 側にのみ記録される。
