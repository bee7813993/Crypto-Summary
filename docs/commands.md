...
| `nexo_spot` | Nexo Pro スポット取引（貯蓄・先物とは別サービス） |
...
- 入出金履歴 = PBR 口座への入金・出金の唯一の情報源（全期間）。
  入庫/出庫に加え、利息・返還利息・プレミアム満期を REWARD として記録する。
  貸出・返還は貸出準備ウォレットとの内部移動なのでスキップする。
- 貸出日次レポート = 利確利息のみ。貸出数量／返還数量は計上しない
  （入出金履歴の入庫／内部移動と同一イベントで二重計上になる）。

そのため日次レポートだけを取り込んだ場合、そのソースの残高は
「受け取った利息の累計」であって預入資産ではない。

2026-03-03 以降の日次レポートは取り込まないこと。「返還受取利息（利確数量）」と
入出金履歴の「返還利息」は同一イベントだが raw_key が異なり重複排除が効かない。

当年分は公式 CSV が出ないため、クローラー連携（`sync-pbr`）で取り込む。

---

## sync-pbr — PBR Lending クローラー同期

```
crypto-summary sync-pbr [--crawl-dir DIR] [--force] [--dry-run]
                        [--purge-year YYYY] [--yes]
```

ローカルのクローラー（PBRLending-History-Check）の出力を読み込んで台帳へ取り込む。
`--crawl-dir` を省略すると環境変数 `PBR_CRAWL_DIR` を使う。

読むファイルは 2 種類。どちらか一方だけでも取り込める。

| ファイル | 内容 | 取り込む範囲 |
|---|---|---|
| `pbrlending_crawled.latest.json` | クロール結果（当年分） | 全部 |
| `viewer_ledger.json` / `viewer_transfers.json` | クローラー画面へ手動インポートした公式データ | クロール期間の**外側**（前も後ろも）。クロール結果が無ければ全期間 |

クロールしていなくても、クローラー画面に公式 CSV を読ませてあれば取り込める
（その場合はヘルスチェックを行わず、手動インポート分だけを取り込む）。
...
# 取り込む（対象期間を洗い替える）
crypto-summary sync-pbr
```

**洗い替え方式**: クローラーは同じ期間を何度でも取り直し、過去日の利率訂正も
反映される。そのため追記ではなく「取り込む期間を丸ごと置き換える」。
何度実行しても結果は同じで、行が消えた場合にも追随する。

削除範囲は *展開後の取引が実在する期間* に限る。他ソースの行や Web の手動入力行
（`manual:` で始まる ID）には触れない。

削除範囲はソースごとに変える:

- `pbr_crawl`（自前のデータ）は取り込む全期間
- `pbr`（公式 CSV を直接取り込んだもの）は**クロールがカバーする期間だけ**、
  かつ**当年の窓のときだけ**。当年に公式データは存在せず、そこにあるのは
  手作り CSV で取り込んだクロール期間のデータなので重複を避けて掃除する。
  過年度に直接取り込んだ公式データは消さない。

**二重計上の防止**: ビューア側の手動インポートは、クロールがカバーする期間を
除いて取り込む（同じ期間はクロール結果を正とする）。さらに、その年に
`pbr` の取引が 1 件でもあれば、ビューア側の同じ年は丸ごとスキップする
（`skip_reasons` に `official_import_exists:<年>` として計上）。
公式 CSV はクローラー画面・Crypto-Summary のどちらに読ませてもよく、
両方に読ませても重複しない。

**再取り込みの判定**: 取り込み元のファイル（クロール結果と `viewer_*.json`）の
更新時刻・サイズと `runId` をまとめた指紋を同期記録に持つ。前回から変わって
いれば、Web 画面を開いたときに自動で取り込み直す。クロールしていなくても、
クローラー画面に公式 CSV を読ませるだけで反映される。

**ヘルスチェック**: `last_crawl.json` の `phase` が `done` でない、または
`failedCurrencies` が空でない場合は取り込まない（少なくとも 1 通貨の利息が
欠けている可能性があるため）。内容を確認したうえで `--force` で強制できる。
クロール結果のファイル自体が無い場合は `--force` でも実行できない。

**残高照合**: 取り込み後、台帳の PBR 残高（`pbr` + `pbr_crawl` の合算）と
クローラーが記録したサイト実残高を突き合わせて表示する。差分は「契約内で
発生済みだが、まだウォレットに付与されていない利息」であることが多い。

| 状態 | 意味 |
|---|---|
| 一致 | 差分なし |
| 未収利息の範囲内 | 差分が未収利息以内 — 通常はこれ |
| 要確認 | 差分が未収利息を超える — データの欠落・重複を疑う |

照合は情報提供のみで、結果によって同期が失敗することはない。

**年次パージ（公式 CSV への移行）**: 年が明けて公式の年間履歴 CSV が公開されたら、

```bash
crypto-summary sync-pbr --purge-year 2026        # クロール由来のみ削除
crypto-summary import --file 2026.csv --exchange pbr   # 公式データを取り込む
```

削除対象は `pbr_crawl` の当該年だけで、公式 CSV 由来の `pbr` には触れない。
