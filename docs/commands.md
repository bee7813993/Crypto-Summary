...
ローカルのクローラー（PBRLending-History-Check）が出力した正規化 JSON
`outputs/pbrlending_crawled.latest.json` を読み込み、当年分を台帳へ取り込む。
...
反映される。そのため追記ではなく「クロールが扱う期間を丸ごと置き換える」。
何度実行しても結果は同じで、行が消えた場合にも追随する。

削除範囲は *展開後の取引が実在する期間* に限る。旧システム期間（〜2026-03-02）や
他ソースの行、Web の手動入力行（`manual:` で始まる ID）には触れない。

**当年の窓では** ソース `pbr_crawl` に加えて同期間の `pbr` 行も掃除する
（当年に公式データは存在せず、そこにあるのは手作り CSV で取り込んだ
クロール期間のデータなので、重複を避けるため）。
**過去年の窓では** `pbr_crawl` のみを削除し、公式 CSV 由来の `pbr` には触れない。
年次パージ → 公式 CSV 取り込みのあとに古いクロール結果を同期しても、
公式データを失わない。
