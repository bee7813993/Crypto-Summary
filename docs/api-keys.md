...
- **「ウォレットアドレス」タブ** — EVM / Solana ウォレットを登録
  1. 上部の「スキャン用 API キー」に Etherscan キー / Helius キーを**一度だけ**登録（全ウォレット共通）
  2. ウォレットアドレスを入力（秘密鍵は不要・絶対に入力しないこと）して「登録」
  3. 一覧の「同期」ボタンで取得

> Etherscan / Helius のキーはアカウント単位の共通設定です。ウォレットごとに入力する必要はありません。
> キーの解決順は「（あれば）ウォレット個別キー → 共通のスキャン用キー → 環境変数」です。
