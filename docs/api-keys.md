...
> ⚠️ **一度 Web に保存すると、以降 `.env` の値は使われません**（Web 保存が優先）。
> `.env` を直しても反映されないときは、Web に古い値が残っていないか
> 管理者設定のステータス表示（`✓ 設定済み` = Web 保存あり / `✓ 環境変数で設定済み` = env 使用中）で確認してください。
> Web の値を直すには、正しいキーを入力して保存し直します（上書きされます）。
> どのキーが実際に使われたかは、認証エラー時のメッセージに先頭・末尾・桁数がマスク表示されます。
...
無料プランで Base / Optimism を含むウォレットを同期すると、この2チェーンは
`Free API access is not supported for this chain` を返します。キーが正しくても
必ず起きる制限なので、**対象外扱いとして同期結果には出しません**（毎回エラーが
出続けるのを避けるため）。対応チェーンのデータは通常どおり取得されます。
1件も取得できなかったときだけ、対象外のチェーンがあったことを結果に添えます。

一度「このプランでは読めない」と分かったチェーンは記録され、以降の同期では
呼び出しません（ウォレットの数だけ無駄な往復が積み上がるのを避けるため）。

**プランをアップグレードしたら**、管理者設定の「スキャン用 API キー」にある
**「対応チェーンを再確認」**を押してください。記録を捨てて、次の同期で全チェーンを
確認し直します（キーを保存し直した場合とアプリ再起動時も自動で捨てられます）。

> EVM ウォレットの同期は全 EVM チェーンを横断スキャンします。`Invalid API Key` が出る場合はキーの値が間違っているか未設定です。

### 同期に時間がかかる場合

Etherscan は初回（キャッシュ未ヒット）の応答が重く、Polygon で 30 秒近く
かかることがあります。タイムアウトしても最大3回まで自動で再試行します。

それでも一部のチェーンだけ失敗した場合は、他チェーンのデータを取り込んだ上で
同期結果に「⚠ 注意」として表示します。**そのチェーンの取引は欠けている**ため、
時間を置いて同期し直してください。
