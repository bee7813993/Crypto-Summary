...
// 同期は成功したが一部チェーンが取れなかったときの注意書き。
// 取引が欠けたまま「成功」とだけ出すと欠落に気づけないため必ず添える。
function syncWarningHtml(warnings) {
  if (!warnings || warnings.length === 0) return "";
  const detail = escapeHtml(warnings.join(" / "));
  return `<div class="sync-detail warn">${t("status.syncWarning", { detail })}</div>`;
}
...
async function loadWalletsTable() {
  ...
  try {
    ...
    tbody.innerHTML = "";
    if (!data.wallets || data.wallets.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6" class="muted">${t("label.noWallets")}</td></tr>`;
      return;
    }
    data.wallets.forEach((w) => {
      const tr = document.createElement("tr");
      const registeredAt = w.created_at ? fmtDate(w.created_at) : "-";
      const lastTx = w.last_tx_ts ? fmtDate(w.last_tx_ts) : "-";
      ...
      tr.innerHTML = `
        ...
        <td style="white-space:nowrap">${escapeHtml(registeredAt)}</td>
        <td style="white-space:nowrap">${escapeHtml(lastTx)}</td>
        <td style="white-space:nowrap;display:flex;gap:6px;align-items:center">
      ...
      tr.querySelector(".btn-wallet-sync").addEventListener("click", async (e) => {
        ...
        try {
          ...
          result.className = "settings-result ok";
          result.innerHTML =
            escapeHtml(t("status.walletSyncDone", { fetched: d.fetched, imported: d.imported, id: w.source_id }))
            + syncWarningHtml(d.warnings);
          loadImportAccountsTable();
          loadWalletsTable();  // 最終取引日を更新
          _txAccountsLoaded = false;
        } catch (err) {
          result.className = "settings-result err";
          result.textContent = t("status.apiSyncFail", { error: err.message });
      ...
      _appendDetailToggle(tr, [
        { label: t("th.address"), value: `<code>${escapeHtml(w.address)}</code>` },
        { label: t("th.chain"), value: escapeHtml(w.chain_label || w.chain) },
        { label: t("th.registeredAt"), value: escapeHtml(registeredAt) },
      ]);
      tbody.appendChild(tr);
    });
  } catch (e) {
...
// 全ウォレット一括同期。API のレート制限があるため逐次実行し、
// 1件失敗しても残りを継続して最後に結果を集約表示する。
const _walletSyncAllBtn = document.getElementById("wallet-sync-all-btn");
if (_walletSyncAllBtn) {
  _walletSyncAllBtn.addEventListener("click", async () => {
    const btn = _walletSyncAllBtn;
    if (btn.disabled) return;
    const result = document.getElementById("import-wallet-result");

    let wallets;
    try {
      wallets = (await fetchJSON("/api/wallets")).wallets || [];
    } catch (e) {
      result.className = "settings-result err";
    ...
    if (wallets.length === 0) {
      ...
      result.textContent = t("status.walletSyncAllNone");
      result.classList.remove("hidden");
      return;
    }
    ...
    // 実行中は行内の同期/削除ボタンも押せないようにする
    document.querySelectorAll("#import-wallets-table .btn-wallet-sync, #import-wallets-table .btn-wallet-delete")
    ...
    try {
      for (let i = 0; i < wallets.length; i++) {
        const w = wallets[i];
        result.textContent = t("status.walletSyncAllProgress", {
          done: i + 1, total: wallets.length, id: w.source_id,
        });
        try {
          ...
          const d = await resp.json();
          if (!resp.ok) throw new Error(d.detail || `HTTP ${resp.status}`);
          results.push({ source_id: w.source_id, ok: true, fetched: d.fetched,
                         imported: d.imported, warnings: d.warnings || [] });
        } catch (e) {
          results.push({ source_id: w.source_id, ok: false, error: e.message });
        }
      }

      const succeeded = results.filter((r) => r.ok);
      const failed = results.filter((r) => !r.ok);
      result.className = "settings-result " + (failed.length === 0 ? "ok" : "err");
      let msg = t("status.syncAllDone", {
        succeeded: succeeded.length, total: results.length,
        imported: succeeded.reduce((s, r) => s + r.imported, 0),
        fetched: succeeded.reduce((s, r) => s + r.fetched, 0),
      });
      if (failed.length > 0) {
        const detail = failed.map((r) => `${r.source_id}: ${r.error}`).join(" / ");
        msg += `<div class="sync-detail">${t("status.syncAllFailed", { count: failed.length, detail: escapeHtml(detail) })}</div>`;
      }
      msg += syncWarningHtml(
        succeeded.flatMap((r) => (r.warnings || []).map((w) => `${r.source_id}: ${w}`)));
      result.innerHTML = msg;
    } finally {
      btn.disabled = false;
      btn.textContent = origText;
      loadWalletsTable();  // 最終取引日を更新（行内ボタンの disabled も解除される）
      loadImportAccountsTable();
      _txAccountsLoaded = false;
    }
  });
...
// システム設定: 対応チェーン再確認ボタン（プランを上げたとき用）
const _recheckChainsBtn = document.getElementById("recheck-chains-btn");
if (_recheckChainsBtn) {
  _recheckChainsBtn.addEventListener("click", async () => {
    const result = document.getElementById("system-keys-result");
    result.classList.add("hidden");
    _recheckChainsBtn.disabled = true;
    try {
      const resp = await fetch(apiUrl("/api/system-keys/recheck-chains"), { method: "POST" });
      const d = await resp.json();
      if (!resp.ok) throw new Error(d.detail || `HTTP ${resp.status}`);
      result.className = "settings-result ok";
      result.textContent = t("status.recheckChainsDone");
    } catch (e) {
      result.className = "settings-result err";
      result.textContent = t("status.systemKeysFail", { error: e.message });
    } finally {
      result.classList.remove("hidden");
      _recheckChainsBtn.disabled = false;
    }
  });
}
...
document.getElementById("sync-all-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (d.total === 0) {
      ...
      msg += syncWarningHtml(
        d.results.flatMap((r) => (r.warnings || []).map((w) => `${r.source_id}: ${w}`)));
      result.innerHTML = msg;
    }
    // 取り込み結果を画面に反映
    loadAccountsPage();
