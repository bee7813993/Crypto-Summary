...
# アダプタが直接参照する列。同じフォルダに置きがちな別形式のCSV
# （Summ/Koinly のエクスポート等）を誤って選ぶと KeyError になるため、
# 読み込み前にヘッダーを検証して内容の分かるエラーを出す。
_REQUIRED_COLUMNS = (
    "日時", "精算区分", "日本円受渡金額", "注文ID", "銘柄名",
    "売買区分", "約定数量", "授受区分", "数量",
)
    ...
    def load(self, path: Path) -> list[CanonicalTx]:
        ...
        with open(path, encoding="utf-8-sig", newline="") as f:
            ...
            require_columns(
                reader.fieldnames,
                _REQUIRED_COLUMNS,
                "GMOコインの取引レポートCSV（取引履歴からダウンロードする年別の yyyy_trading_report.csv）",
            )
