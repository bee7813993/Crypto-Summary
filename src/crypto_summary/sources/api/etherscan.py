...
# 1 リクエストの待ち時間。Etherscan は初回（キャッシュ未ヒット）の応答が重く、
# polygon で 26〜30 秒かかることを実測している（応答後は 0.5 秒程度に落ち着く）。
# 30 秒だとその初回にぶつかって取りこぼすため、余裕を持たせる。
_TIMEOUT = 60.0
# タイムアウト・接続断・サーバエラー時の試行回数（初回を含む）。
_MAX_ATTEMPTS = 3
# リトライ前の待機秒数（2 回目・3 回目…の順に使い、足りなければ最後の値を繰り返す）。
_RETRY_WAIT = (1.0, 3.0)
# 契約プランではそのチェーンを読めないときに API が返す文言の一部。
# 例: "Free API access is not supported for this chain. Please upgrade your api plan..."
_UNSUPPORTED_CHAIN_HINT = "not supported for this chain"


class ChainNotSupportedError(RuntimeError):
    """契約中の API プランでは取得できないチェーン（無料枠の Base / Optimism など）。

    キーのプランを上げない限り毎回同じ応答になるため、呼び出し側は
    「失敗」ではなく「対象外」として扱う。毎回エラーとして見せない。
    """
# 1 リクエストあたりの最大取得件数。
...
class EtherscanApiSource(ArbiscanCsvSource):
    ...
    def _fetch(self, params: dict[str, Any], action: str) -> httpx.Response:
        """一時的な失敗（タイムアウト・接続断・5xx・429）はリトライして応答を返す。

        1 回のタイムアウトで諦めると、そのチェーンの取引が丸ごと欠ける。
        しかも他チェーンが 1 件でも取れていれば同期は成功として報告されるため、
        欠けたことに気づけない。時間を置けば直る類の失敗は再試行する。

        API が応答を返した上でのエラー（無料プラン非対応など）は待っても
        変わらないのでリトライしない。呼び出し元がそのまま例外にする。
        """
        last_error: Exception | None = None
        for attempt in range(_MAX_ATTEMPTS):
            if attempt:
                time.sleep(_RETRY_WAIT[min(attempt - 1, len(_RETRY_WAIT) - 1)])
            time.sleep(_RATE_LIMIT_SLEEP)
            try:
                resp = httpx.get(_BASE, params=params, timeout=self.timeout)
            except httpx.TransportError as e:  # タイムアウト・接続断など
                last_error = e
                continue
        ...
        raise RuntimeError(
            f"Etherscan API error ({action}): {_MAX_ATTEMPTS} 回試行しても"
            f"応答がありませんでした（1 回あたり {self.timeout:.0f} 秒待機）: {last_error}"
        )
    ...
    def _request(self, action: str, startblock: int) -> list[dict[str, Any]]:
        ...
        data = self._fetch(params, action).json()
        ...
        if data.get("status") != "1":
            ...
            if _UNSUPPORTED_CHAIN_HINT in str(result).lower():
                raise ChainNotSupportedError(str(result))
