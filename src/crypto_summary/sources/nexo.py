...
Nexo Pro は Nexo 本体アカウント（貯蓄・先物）とは別サービス。
本体側の CSV は nexo_savings / nexo_futures アダプタが扱う。
...
_SPOT_REQUIRED = ("timestamp", "pair", "filledAmount", "executedPrice")
...
class NexoSpotCsvSource(CsvSourceAdapter):
    ...
    def load(self, path: Path) -> list[CanonicalTx]:
        ...
        with open(path, encoding="utf-8-sig", newline="") as f:
            ...
            require_columns(
                reader.fieldnames, _SPOT_REQUIRED,
                "Nexo Pro のスポット取引履歴CSV（SpotHistory）",
            )
...
_DNW_REQUIRED = ("timestamp", "amount", "asset", "side")
...
class NexoDnWCsvSource(CsvSourceAdapter):
    ...
    def load(self, path: Path) -> list[CanonicalTx]:
        ...
        with open(path, encoding="utf-8-sig", newline="") as f:
            reader = csv.DictReader(f)
            require_columns(
                reader.fieldnames, _DNW_REQUIRED,
                "Nexo Pro の入出金履歴CSV（DnWHistory）",
            )
            for i, row in enumerate(reader):
                tx = self._parse_row(row, i)
                if tx is not None:
...
class NexoProCsvSource(CsvSourceAdapter):
    ...
    SpotHistory（スポット取引）/ DnWHistory（入出金）を判別し、対応する
    パーサーへ委譲する。Nexo 本体アカウントの CSV（貯蓄口座の取引明細・
    先物取引履歴）は別サービスのため、ここでは取り込まず選ぶべき取引所を
    案内して落とす。
    """
    ...
    def load(self, path: Path) -> list[CanonicalTx]:
        ...
        if looks_like_futures(header):
            raise ValueError(
                "これは Nexo の先物取引履歴CSVです（Nexo Pro とは別サービス）。"
        ...
        raise ValueError(
            ...
            "貯蓄口座の取引明細は「Nexo（貯蓄口座）」、"
            "先物取引履歴は「Nexo（先物取引）」を選択してください。"
            f" ヘッダー: {', '.join(header)}"
        )
