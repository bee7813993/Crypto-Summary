...
def balances_as_of(
    ...
    """その日の終わりの時点の資産別残高。daily_balances の同じ日の値と一致する。

    日次系列を組み立てず SQL 1本で済ませる。「前日の残高」「30日前の残高」の
    ように特定の1日だけ要るとき（前日比や N日差の基準）に使う。
    """
    bals = ledger.balances(
        source=source,
        until=datetime.combine(day, time.max, tzinfo=timezone.utc),
        exclude_labels=exclude_labels,
    )
    # ゼロ残高は含めない（daily_balances と同じ扱い。売り切った資産が
    # 「0 を持っている」ように見えないようにする）
    return {
        a: v
        for a, v in bals.items()
        if v != Decimal(0) and (not asset or a.upper() == asset.upper())
    }
