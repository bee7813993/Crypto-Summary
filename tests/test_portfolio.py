...
def test_balances_as_of_matches_daily_balances(ledger):
    """1日だけ引く版と日次系列版が同じ値を返す（前日比の基準がずれない）。"""
    ledger.upsert(_tx("d1", "ex", _dt(2024, 1, 1), TxType.DEPOSIT, ra="BTC", rv=1))
    ledger.upsert(_tx("d2", "ex", _dt(2024, 1, 3), TxType.DEPOSIT, ra="BTC", rv=0.5))
    ledger.upsert(_tx("d3", "ex", _dt(2024, 1, 3), TxType.DEPOSIT, ra="ETH", rv=2))
    series = pf.daily_balances(ledger, start=date(2024, 1, 1), end=date(2024, 1, 5))
    for day in (date(2024, 1, 1), date(2024, 1, 2), date(2024, 1, 3), date(2024, 1, 5)):
        assert pf.balances_as_of(ledger, day) == series[day.isoformat()]


def test_balances_as_of_omits_zero_balances(ledger):
    """売り切った資産は載せない（daily_balances と同じ扱い）。"""
    ledger.upsert(_tx("d1", "ex", _dt(2024, 1, 1), TxType.DEPOSIT, ra="BTC", rv=1))
    ledger.upsert(_tx("w1", "ex", _dt(2024, 1, 2), TxType.WITHDRAW, sa="BTC", sv=1))
    assert pf.balances_as_of(ledger, date(2024, 1, 1)) == {"BTC": Decimal("1")}
    assert pf.balances_as_of(ledger, date(2024, 1, 2)) == {}
    series = pf.daily_balances(ledger, start=date(2024, 1, 1), end=date(2024, 1, 2))
    assert "2024-01-02" not in series          # 全部ゼロの日は日次系列にも出ない


def test_balances_as_of_ignores_later_transactions(ledger):
    """その日より後の取引は入らない（当日ぶんは日付の終わりまで入る）。"""
    ledger.upsert(_tx("d1", "ex", _dt(2024, 1, 1), TxType.DEPOSIT, ra="BTC", rv=1))
    ledger.upsert(_tx("d2", "ex", _dt(2024, 1, 2, 23), TxType.DEPOSIT, ra="BTC", rv=2))
    ledger.upsert(_tx("d3", "ex", _dt(2024, 1, 3), TxType.DEPOSIT, ra="BTC", rv=4))
    assert pf.balances_as_of(ledger, date(2024, 1, 1))["BTC"] == Decimal("1")
    assert pf.balances_as_of(ledger, date(2024, 1, 2))["BTC"] == Decimal("3")
    assert pf.balances_as_of(ledger, date(2023, 12, 31)) == {}


def test_balances_as_of_filters(ledger):
    """source / asset の絞り込みが効く。"""
    ledger.upsert(_tx("d1", "a", _dt(2024, 1, 1), TxType.DEPOSIT, ra="BTC", rv=1))
    ledger.upsert(_tx("d2", "b", _dt(2024, 1, 1), TxType.DEPOSIT, ra="ETH", rv=5))
    assert set(pf.balances_as_of(ledger, date(2024, 1, 1), source="a")) == {"BTC"}
    assert set(pf.balances_as_of(ledger, date(2024, 1, 1), asset="eth")) == {"ETH"}
