...
def test_wrong_format_csv_gives_clear_error(tmp_path):
    """別形式のCSV（Summ等のエクスポート）は KeyError('精算区分') ではなく、
    欠けている列・実際のヘッダー・正しいファイルの案内を含む ValueError になること。"""
    p = tmp_path / "summ_Gmo_Coin.csv"
    p.write_text(
        "Timestamp (UTC),Type,Base Currency,Base Amount\n"
        "2026-01-19 02:51:00,buy,SOL,1.0\n",
        encoding="utf-8-sig",
    )
    with pytest.raises(ValueError) as ei:
        GmoCsvSource("gmo").load(p)
    msg = str(ei.value)
    assert "精算区分" in msg              # 何が欠けているか
    assert "Timestamp (UTC)" in msg       # 実際のヘッダー
    assert "取引レポート" in msg          # どのファイルを選ぶべきか


def test_empty_csv_gives_clear_error(tmp_path):
    """空ファイルもヘッダー検証で内容の分かるエラーになること。"""
    p = tmp_path / "empty.csv"
    p.write_text("", encoding="utf-8")
    with pytest.raises(ValueError, match="ヘッダーなし"):
        GmoCsvSource("gmo").load(p)
