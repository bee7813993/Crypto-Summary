...
def test_wallet_list_includes_last_tx_ts(client, db_path):
    """一覧に取引データの最新 timestamp（最終取引日）が含まれる。"""
    addr = "0x" + "d" * 40
    client.post("/api/wallets", json={"address": addr, "source_id": "w_tx"})

    led = Ledger(db_path)
    led.upsert(_deposit("w_tx", "BTC", "1", 5))
    led.upsert(_deposit("w_tx", "ETH", "1", 9))  # こちらが最新
    led.close()

    wallets = client.get("/api/wallets").json()["wallets"]
    w = next(x for x in wallets if x["source_id"] == "w_tx")
    assert w["last_tx_ts"] == datetime(2024, 1, 9, tzinfo=timezone.utc).isoformat()


def test_wallet_list_last_tx_ts_none_without_txs(client):
    """取引データが無いウォレットの最終取引日は None。"""
    client.post("/api/wallets", json={"address": "0x" + "e" * 40, "source_id": "w_empty"})
    wallets = client.get("/api/wallets").json()["wallets"]
    w = next(x for x in wallets if x["source_id"] == "w_empty")
    assert w["last_tx_ts"] is None
...
def _fake_etherscan_by_chain(monkeypatch, behavior):
    """チェーン名ごとの挙動を差し込む。値は取引リストか送出する例外。"""
    import crypto_summary.sources.api.etherscan as es

    by_id = {es.CHAIN_IDS[name]: v for name, v in behavior.items()}

    class FakeEtherscan:
        def __init__(self, source_id, address, key, chain_id):
            self._outcome = by_id.get(chain_id, [])
...
def test_wallet_sync_warns_when_one_chain_fails(client, monkeypatch):
    """他チェーンが取れていても、落ちたチェーンは警告として返す。

    黙って成功にすると、そのチェーンの取引が欠けたことに気づけない。
    """
    monkeypatch.setenv("ETHERSCAN_API_KEY", "DUMMYKEY")
    client.post("/api/wallets", json={"address": "0x" + "c" * 40, "source_id": "evm_warn"})
    _fake_etherscan_by_chain(monkeypatch, {
        "ethereum": [_deposit("evm_warn", "ETH", "1", 5)],
        "polygon": RuntimeError("3 回試行しても応答がありませんでした"),
    })

    d = client.post("/api/wallets/evm_warn/sync").json()
    assert d["imported"] == 1
    assert len(d["warnings"]) == 1
    assert "polygon" in d["warnings"][0]


def test_wallet_sync_does_not_warn_about_unsupported_chains(client, monkeypatch):
    """プラン非対応のチェーンは失敗ではないので、取引が取れていれば黙っている。

    無料プランでは毎回必ず起きるため、警告にすると常時ノイズになる。
    """
    monkeypatch.setenv("ETHERSCAN_API_KEY", "DUMMYKEY")
    client.post("/api/wallets", json={"address": "0x" + "d" * 40, "source_id": "evm_plan"})
    _fake_etherscan_by_chain(monkeypatch, {
        "ethereum": [_deposit("evm_plan", "ETH", "1", 6)],
        "base": es_unsupported(),
        "optimism": es_unsupported(),
    })

    d = client.post("/api/wallets/evm_plan/sync").json()
    assert d["imported"] == 1
    assert d["warnings"] == []


def test_wallet_sync_mentions_unsupported_chains_when_empty(client, monkeypatch):
    """1件も取れなかったときだけ、対象外チェーンがあることを伝える。"""
    monkeypatch.setenv("ETHERSCAN_API_KEY", "DUMMYKEY")
    client.post("/api/wallets", json={"address": "0x" + "f" * 40, "source_id": "evm_none"})
    _fake_etherscan_by_chain(monkeypatch, {
        "base": es_unsupported(),
        "optimism": es_unsupported(),
    })

    d = client.post("/api/wallets/evm_none/sync").json()
    assert d["fetched"] == 0
    assert len(d["warnings"]) == 1
    assert "base" in d["warnings"][0] and "optimism" in d["warnings"][0]


def test_unsupported_chain_is_not_requested_again(client, monkeypatch):
    """プラン非対応と分かったチェーンは、次の同期から呼び出さない。

    無料プランの Base / Optimism はウォレットごと・同期ごとに必ず弾かれるため、
    毎回叩くと同期1回につき無駄な往復が積み上がる。
    """
    monkeypatch.setattr(web_app, "_unsupported_chains", {})
    monkeypatch.setenv("ETHERSCAN_API_KEY", "DUMMYKEY")
    client.post("/api/wallets", json={"address": "0x" + "1" * 40, "source_id": "evm_c1"})
    client.post("/api/wallets", json={"address": "0x" + "2" * 40, "source_id": "evm_c2"})
    ...
    class FakeEtherscan:
        ...
        def fetch_all(self, record_gas=True):
            tried.append(self._chain_id)
            if self._chain_id in (es.CHAIN_IDS["base"], es.CHAIN_IDS["optimism"]):
                raise es_unsupported()
            return []

    monkeypatch.setattr(es, "EtherscanApiSource", FakeEtherscan)

    client.post("/api/wallets/evm_c1/sync")
    first = list(tried)
    tried.clear()
    client.post("/api/wallets/evm_c2/sync")

    assert len(first) == len(es.CHAIN_IDS)          # 1件目は全チェーン試す
    assert es.CHAIN_IDS["base"] not in tried        # 2件目は覚えた分を飛ばす
    assert es.CHAIN_IDS["optimism"] not in tried
    assert es.CHAIN_IDS["ethereum"] in tried        # 対応チェーンはこれまで通り


def test_recheck_chains_clears_memory(client, monkeypatch):
    """管理者設定から再確認すると、覚えた非対応チェーンを忘れる。"""
    monkeypatch.setattr(web_app, "_unsupported_chains", {"abc123": {8453, 10}})

    d = client.post("/api/system-keys/recheck-chains").json()
    assert d["cleared"] == 2
    assert web_app._unsupported_chains == {}


def test_saving_keys_forgets_unsupported_chains(client, monkeypatch):
    """キーを入れ替えたらプランも変わりうるので、記録を持ち越さない。"""
    from cryptography.fernet import Fernet

    monkeypatch.setattr(web_app, "_unsupported_chains", {"abc123": {8453}})
    monkeypatch.setenv("CS_SECRET_KEY", Fernet.generate_key().decode())

    r = client.post("/api/system-keys", json={"etherscan": "NEWKEY"})
    assert r.status_code == 200
    assert web_app._unsupported_chains == {}


def test_wallet_sync_errors_when_all_chains_fail(client, monkeypatch):
    """1件も取れず本当の失敗しかないときは、これまで通りエラーにする。"""
    monkeypatch.setenv("ETHERSCAN_API_KEY", "DUMMYKEY")
    client.post("/api/wallets", json={"address": "0x" + "b" * 40, "source_id": "evm_dead"})
    import crypto_summary.sources.api.etherscan as es
    _fake_etherscan_by_chain(monkeypatch, {
        name: RuntimeError("boom") for name in es.CHAIN_IDS
    })

    r = client.post("/api/wallets/evm_dead/sync")
    assert r.status_code == 502
    assert "Etherscan API エラー" in r.json()["detail"]


def es_unsupported():
    """無料プランで読めないチェーンの応答を模した例外。"""
    from crypto_summary.sources.api.etherscan import ChainNotSupportedError

    return ChainNotSupportedError(
...
def test_summary_prev_value_uses_the_previous_days_balance(prev_client):
    """prev_value = 前日の残高 × 前営業日の終値。

    この台帳は前日から残高が動いていないので、いまの残高と同じ数量で出る。
    """
    d = prev_client.get("/api/summary?currency=USD").json()
    btc = next(a for a in d["assets"] if a["asset"] == "BTC")
    assert btc["prev_date"] == _days_ago(1)
    assert Decimal(btc["prev_price"]) == Decimal("55000")
    assert Decimal(btc["prev_balance"]) == Decimal("0.5")
...
def test_balances_endpoint_returns_the_balance_on_that_day(client):
    """指定日の終わり時点の残高。価格は付かない（CoinGecko も叩かない）。"""
    d = client.get("/api/balances?as_of=2024-01-02").json()
    assert d["as_of"] == "2024-01-02"
    got = {a["asset"]: Decimal(a["balance"]) for a in d["assets"]}
    assert got == {"BTC": Decimal("0.5"), "ETH": Decimal("2")}   # SOL は 1/3 から
    assert "price" not in d["assets"][0]


def test_balances_endpoint_defaults_to_today_and_takes_scopes(client):
    """as_of 省略は今日。scope で口座・資産に絞れる（履歴APIと同じ書式）。"""
    d = client.get("/api/balances").json()
    assert d["as_of"] == date.today().isoformat()
    assert {a["asset"] for a in d["assets"]} == {"BTC", "ETH", "SOL", "MYSTERY"}

    only_btc = client.get("/api/balances?scope=asset:btc").json()
    assert [a["asset"] for a in only_btc["assets"]] == ["BTC"]

    # 口座は履歴APIと同じく「表示名」で指定する（acct_b → Acct B）
    acct_b = client.get("/api/balances?scope=account:Acct%20B").json()
    assert {a["asset"] for a in acct_b["assets"]} == {"SOL", "MYSTERY"}


def test_balances_endpoint_rejects_a_bad_date(client):
    assert client.get("/api/balances?as_of=2024-13-99").status_code == 422


def test_balances_endpoint_supports_n_day_differences(client):
    """N日前の残高が引けるので、数量の差を利用側で作れる。"""
    old = client.get("/api/balances?as_of=2024-01-01").json()
    new = client.get("/api/balances?as_of=2024-01-03").json()
    def _qty(payload, sym):
        return next(
            (Decimal(a["balance"]) for a in payload["assets"] if a["asset"] == sym),
            Decimal("0"),
        )
    assert _qty(new, "SOL") - _qty(old, "SOL") == Decimal("10")   # この2日で +10


def _tx_at(source: str, asset: str, amount: str, when: datetime, out: bool = False):
    """任意の日時の入出庫（前日比の境界を跨ぐ検証用）。"""
    kind = "w" if out else "d"
    return CanonicalTx(
        id=CanonicalTx.make_id(source, f"{kind}{asset}{when.isoformat()}"),
        source=source,
        timestamp=when,
        type=TxType.WITHDRAW if out else TxType.DEPOSIT,
        **(
            {"sent_asset": asset, "sent_amount": Decimal(amount)} if out
            else {"received_asset": asset, "received_amount": Decimal(amount)}
        ),
    )


@pytest.fixture
def moving_client(tmp_path, monkeypatch) -> TestClient:
    """今日 BTC を 0.5 買い足し、ETH を全部送り出した台帳。"""
    now = datetime.now(timezone.utc)
    db = Ledger(tmp_path / "moving.db")
    db.upsert(_deposit("acct_a", "BTC", "0.5", 1))       # 2024-01-01
    db.upsert(_deposit("acct_a", "ETH", "2", 2))         # 2024-01-02
    db.upsert(_tx_at("acct_a", "BTC", "0.5", now))       # 今日 買い足し
    db.upsert(_tx_at("acct_a", "ETH", "2", now, out=True))  # 今日 全部送出
    db.close()

    monkeypatch.setattr(
        web_app, "fetch_prices",
        lambda assets, currency, warn=None: {
            a.upper(): p for a, p in
            (("BTC", Decimal("60000")), ("ETH", Decimal("3000")))
            if a.upper() in {x.upper() for x in assets}
        },
    )
    monkeypatch.setattr(web_app, "fetch_price_history", _fake_history({
        "BTC": {_days_ago(1): Decimal("60000")},
        "ETH": {_days_ago(1): Decimal("3000")},
    }))
    return TestClient(web_app.create_app(str(tmp_path / "moving.db")))


def test_summary_prev_value_excludes_todays_purchase(moving_client):
    """今日買い足したぶんは前日値に入らない（前日比に増加として出る）。"""
    d = moving_client.get("/api/summary?currency=USD").json()
    btc = next(a for a in d["assets"] if a["asset"] == "BTC")
    assert Decimal(btc["balance"]) == Decimal("1.0")
    assert Decimal(btc["prev_balance"]) == Decimal("0.5")   # 前日は 0.5 だけ
    assert Decimal(btc["prev_value"]) == Decimal("30000")   # 0.5 * 60000
    # 価格は動いていないので、増減は買い足したぶんそのもの
    assert Decimal(btc["value"]) - Decimal(btc["prev_value"]) == Decimal("30000")


def test_summary_keeps_an_asset_sent_out_today(moving_client):
    """今日ゼロにした資産も前日値を持って残る（受取側だけ増えて見えないように）。"""
    d = moving_client.get("/api/summary?currency=USD").json()
    eth = next(a for a in d["assets"] if a["asset"] == "ETH")
    assert Decimal(eth["balance"]) == Decimal("0")
    assert Decimal(eth["prev_value"]) == Decimal("6000")    # 2 * 3000
    # 保有件数には数えない（前日比のためだけに載っている行）
